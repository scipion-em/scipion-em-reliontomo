...
class ProtRelion5RefineSubtomograms(ProtRelionRefineBase):
    ...
    def _defineReferenceParams(self, form):
        ...
        form.addParam('doResizeRef', BooleanParam,
                      ...
                      help='If true, and if the input reference map (and mask) do not have the same pixel size and/or '
                           'box size, then they will be re-scaled and re-boxed accordingly. If this option is set to '
                           'false, then the program will die with an error if the reference does not have the correct '
                           'pixel and/or box size.')
    ...
    def _genAutoRefineCommand(self):
        ...
        if ref.hasHalfMaps():
            refFile, _ = ref.getHalfMaps().split(',')
